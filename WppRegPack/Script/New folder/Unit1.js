...
function vv(){ 
  Approve_Level[0] = "1707200085*Test Sample - Automation4 23January2019 10:05:10**SSC - Billers"
var LoginEmp = WorkspaceUtils.Credentiallogin(Project.Path+excelName, "userRoles");
  var HRData = ""
...
//  ref.Refresh();
  ...
  for(var j=0;j<budgetTable.getItemCount()-1;j++){  
    ...
    for(var ii=0;ii<table.getItemCount();ii++){ 
      AArrays[ii] = table.getItem(ii).getText_2(0).OleValue.toString().trim();
    }
    ...
    var temp = "0";
    for(var ii=0;ii<table.getItemCount();ii++){  
        if(table.getItem(ii).getText_2(0).OleValue.toString().trim()==uniqueAAray[ij]){
        if(table.getItem(ii).getText_2(3)!=""){
        temp = aqConvert.StrToInt(temp.toString().trim())+aqConvert.StrToInt(table.getItem(ii).getText_2(3).OleValue.toString().trim());
        }
        }
    }
...
function vf(){ 
  var rr = "E1187 - Miscellaneous";
  Log.Message(rr.substring(rr.indexOf(" - ")+3))
}