...
//  Employee_no.Keys("170710134");
  ...
  if(flag){ 
   ...
   checkmark = false;
   company.Click();
   checkmarks = WorkspaceUtils.SearchByValue(company,"Company",userInfo[2]);
   comID = userInfo[2];
if(checkmarks){
ReportUtils.logStep("INFO", "Company has Changed");
checkmark = true;
}else{ 
  ValidationUtils.verify(checkmarks,true,"Company is NOT Available")
}
  ...
  passwordBlocked.Click();
   ...
   ValidationUtils.verify(true,true,"User has Changed");
...
function ChangeUser(){ 
  ...
  ReportUtils.logStep("INFO", "Change User has Strated");
  goToMenu();
