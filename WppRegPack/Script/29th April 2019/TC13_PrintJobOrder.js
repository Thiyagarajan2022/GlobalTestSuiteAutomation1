...
ReportUtils.logStep("INFO", "Printing Job Order for "+Job_Name);
GoToSubJob();
}
...
//        JobID = table.getItem(f).getText_2(2).OleValue.toString().trim()
    ...
    quote.Click();
    Delay(5000)
    var printJob = Sys.Process("Maconomy").SWTObject("Shell", "Deltek Maconomy - *").SWTObject("Composite", "").SWTObject("Composite", "", 3).SWTObject("Composite", "").SWTObject("Composite", "", 3).SWTObject("Composite", "", 1).SWTObject("Composite", "").SWTObject("Composite", "").SWTObject("Composite", "").SWTObject("Composite", "", 5).SWTObject("Composite", "").SWTObject("PTabFolder", "").SWTObject("Composite", "", 2).SWTObject("SingleToolItemControl", "", 5);
    Sys.HighlightObject(printJob);
    if(printJob.isEnabled()){
    printJob.Click();
    ValidationUtils.verify(true,true,"Print Job OrderConfirmation icon is Clicked Document is downloaded")
    }else{ 
      ValidationUtils.verify(false,true,"Print Order Confirmation is Invisible /Status not in ORDER")
//      Log.Warning("Print Order Confirmation is Invisible /Status not in ORDER")
    }
    Delay(3000)
    }
    else{
...
var xlDriver = DDT.ExcelDriver(Project.Path+excelName, "PrintJobOrder", true);
var id =0;
var colsList = [];
