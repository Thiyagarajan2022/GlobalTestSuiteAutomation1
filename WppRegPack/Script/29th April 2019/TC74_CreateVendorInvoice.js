...
//    code.Keys("[Tab]");
  ...
  var save = Sys.Process("Maconomy").SWTObject("Shell", "Deltek Maconomy - *").SWTObject("Composite", "").SWTObject("Composite", "", 3).SWTObject("Composite", "").SWTObject("Composite", "", 3).SWTObject("Composite", "", 1).SWTObject("Composite", "").SWTObject("Composite", "").SWTObject("Composite", "").SWTObject("Composite", "", 5).SWTObject("Composite", "", 2).SWTObject("PTabFolder", "").SWTObject("TabFolderPanel", "", 1).SWTObject("Composite", "", 1).SWTObject("SingleToolItemControl", "", 2);
   if((invoiceDetails[2]!="")|| (invoiceDetails[5]!="")){
  Sys.HighlightObject(save);
  save.Click();
  }else{ 
    ValidationUtils.verify(false,true,"Purchase Order or Vendor No is Need to create VendorInvoice");
  }
Delay(5000);
...
// Getting Details from Invoice Line
    ...
    arrys = excel(sheetName,2)
       ...
       if(sp[sp.length-2].toUpperCase().indexOf("ADD")!=-1){
...
invoiceAllocationLine.Click();
Delay(1000);
Log.Message("Length :"+addArray.length);
for(var i=0;i<addArray.length;i++){  
Log.Message("Adding_item :"+addArray[i]);
}
for(var k=0;k<addArray.length;k++){ 
add = Sys.Process("Maconomy").SWTObject("Shell", "Deltek Maconomy - *").SWTObject("Composite", "").SWTObject("Composite", "", 3).SWTObject("Composite", "").SWTObject("Composite", "", 3).SWTObject("Composite", "", 1).SWTObject("Composite", "").SWTObject("Composite", "").SWTObject("Composite", "").SWTObject("Composite", "", 3).SWTObject("Composite", "", 2).SWTObject("PTabFolder", "").SWTObject("TabFolderPanel", "", 1).SWTObject("Composite", "", 1).SWTObject("SingleToolItemControl", "", 4);
 ...
 if(newItem[0]!=""){
  purchaseOrder.Click();
//  WorkspaceUtils.SearchByValue(purchaseOrder,"Purchase Order Line",newItem[0]);
  var ObjectAddrs = purchaseOrder;
  var popupName = "Purchase Order Line";
  var value = newItem[0];
  var checkmark = false;
   Delay(1000);
    Sys.Desktop.KeyDown(0x11);
...
//    Log.Message(popupName)
//    Log.Message(value)
    var code = Sys.Process("Maconomy").SWTObject("Shell", "Purchase Order Line").SWTObject("Composite", "").SWTObject("Composite", "", 1).SWTObject("Composite", "").SWTObject("McFilterPaneWidget", "").SWTObject("McTableWidget", "", 2).SWTObject("McGrid", "", 2).SWTObject("McValuePickerWidget", "");
    code.setText(value);
    Delay(3000);
    var serch = Sys.Process("Maconomy").SWTObject("Shell", popupName).SWTObject("Composite", "").SWTObject("Composite", "", 1).SWTObject("Composite", "").SWTObject("McFilterPaneWidget", "").SWTObject("McPagingWidget", "", 1).SWTObject("Button", "Search ");
    Sys.HighlightObject(serch);
    serch.Click();
    Delay(5000);
    var table = Sys.Process("Maconomy").SWTObject("Shell", popupName).SWTObject("Composite", "").SWTObject("Composite", "", 1).SWTObject("Composite", "").SWTObject("McFilterPaneWidget", "").SWTObject("McTableWidget", "", 2).SWTObject("McGrid", "");
    Sys.HighlightObject(table);
    var itemCount = table.getItemCount();
    if(itemCount>0){ 
    for(var i=0;i<itemCount;i++){
      if((table.getItem(i).getText_2(0).OleValue.toString().trim()==value) && (table.getItem(i).getText_2(8).OleValue.toString().trim()==newItem[2])){ 
       var OK = Sys.Process("Maconomy").SWTObject("Shell", popupName).SWTObject("Composite", "").SWTObject("Composite", "", 2).SWTObject("Button", "OK")
          OK.Click();
          checkmark = true;
          break;
...
//    code.Keys("[Tab]");
    ...
    serch.Click();
    Delay(5000);
    var table = Sys.Process("Maconomy").SWTObject("Shell", "Job").SWTObject("Composite", "").SWTObject("Composite", "", 1).SWTObject("Composite", "").SWTObject("McFilterPaneWidget", "").SWTObject("McTableWidget", "", 3).SWTObject("McGrid", "")
    Sys.HighlightObject(table);
    var itemCount = table.getItemCount();
    if(itemCount>0){ 
    for(var i=0;i<itemCount;i++){
      ...
      ValidationUtils.verify(false,true,"Job No is Need to create VendorInvoice");
    }
 ...
 if(newItem[2]!=""){
  ...
  WorkspaceUtils.SearchByValue(workcode,"Work Code",newItem[2]);
      ...
      ValidationUtils.verify(false,true,"workcode is Need to create VendorInvoice");
    }
    Sys.Desktop.KeyDown(0x09)
    Delay(2000)
    Sys.Desktop.KeyDown(0x09)
 ...
 if(newItem[3]!=""){
  ...
  workcodeDescription.setText(newItem[3])
      ...
      ValidationUtils.verify(false,true,"workcode Description is Need to create VendorInvoice");
    }
    Delay(2000)
    Sys.Desktop.KeyDown(0x09)
    Sys.Desktop.KeyUp(0x09)
 ...
 if(newItem[8]!=""){
  ...
  WorkspaceUtils.SearchByValue(tax_code_2,"Local Specification 9",newItem[8]);
      ...
      ValidationUtils.verify(false,true,"HSN is Need to create VendorInvoice");
    }
    Delay(2000);
  var save = Sys.Process("Maconomy").SWTObject("Shell", "Deltek Maconomy - *").SWTObject("Composite", "").SWTObject("Composite", "", 3).SWTObject("Composite", "").SWTObject("Composite", "", 3).SWTObject("Composite", "", 1).SWTObject("Composite", "").SWTObject("Composite", "").SWTObject("Composite", "").SWTObject("Composite", "", 3).SWTObject("Composite", "", 2).SWTObject("PTabFolder", "").SWTObject("TabFolderPanel", "", 1).SWTObject("Composite", "", 1).SWTObject("SingleToolItemControl", "", 2);
  save.Click();
...
ValidationUtils.verify(true,true,"Vendor Invoice is Created");
  if(Sys.Process("Maconomy").SWTObject("Shell", "*").WndCaption=="AP Transactions - Invoice Allocation"){
...
function v(){ 
    ...
    arrys = excel(sheetName,2)
       for(var i=0;i<arrys.length;i++){
       var sp = arrys[i].split("*");
       if(sp[sp.length-2].toUpperCase()=="ADD"){ 
       vv = arrys[i].lastIndexOf("*");
       subStr = arrys[i].substring(0,vv);
       vv = subStr.lastIndexOf("*");
//       Log.Message("SubString :"+subStr.substring(0,vv+1));
         addArray[a] = subStr.substring(0,vv+1);
//         addArray[a] = sp[0]+"*"+sp[1]+"*"+sp[2]+"*"+sp[3]+"*"+sp[4]+"*"+sp[5]+"*"+sp[6]+"*"+sp[7]+"*"+sp[8]+"*";
         Log.Message("Add :"+addArray[a]);
         a++;
       }
       if(sp[sp.length-2].toUpperCase()=="REMOVE"){ 
       vv = arrys[i].lastIndexOf("*");
       subStr = arrys[i].substring(0,vv);
       vv = subStr.lastIndexOf("*");
//       Log.Message("SubString :"+subStr.substring(0,vv+1));
