...
var sheetName = "InterCompany Job";
var Language = "";
Indicator.Show();
...
ExcelUtils.setExcelName(workBook, "InterCompany Job", true);
...
Project_manager = ExcelUtils.getRowDatas("Agency - Senior Finance",Intercompany_OpCo);
...
Product = ReadExcelSheet("Inter Company Global Product Number",EnvParams.Opco,"Data Management");
...
function Submiting_Job() {
  ...
  TextUtils.writeLog("Inter Company Job Number :"+table.getItem(v).getText_2(2).OleValue.toString().trim());
  ...
  for(var i=0;i<Approval_Table.getItemCount();i++){   
     var approvers="";
      ...
      approvers = Intercompany_OpCo+"*"+JobID+"*"+ temp;
...
ExcelUtils.setExcelName(workBook,"Data Management", true);
ExcelUtils.WriteExcelSheet("InterCompany Job Number",EnvParams.Opco,"Data Management",JobID)
ExcelUtils.WriteExcelSheet("InterCompany Job Template",EnvParams.Opco,"Data Management",TemplateNo)
}
}
