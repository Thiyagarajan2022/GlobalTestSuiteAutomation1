...
//var pdf = Sys.Process("AcroRd32", 2).Window("AcrobatSDIWindow", "*Invoice"+"*", 1).Window("AVL_AVView", "AVFlipContainerView", 2).Window("AVL_AVView", "AVDocumentMainView", 1).Window("AVL_AVView", "AVTopBarView", 4);
    if(Sys.Process("AcroRd32", 2).Window("AcrobatSDIWindow", "Print Job Invoice"+"*", 1).WndCaption.indexOf("Invoice")!=-1){
    aqUtils.Delay(2000, Indicator.Text);
  //  Sys.HighlightObject(pdf)
    Sys.Desktop.KeyDown(0x12); //Alt
    Sys.Desktop.KeyDown(0x46); //F
...
var obj = JavaClasses.org_apache_pdfbox_util.PDFTextStripper.newInstance();
  ...
  ExcelUtils.setExcelName(workBook,"Data Management", true);
  ExcelUtils.WriteExcelSheet("Invoice from Budget No",EnvParams.Opco,"Data Management",textobj)
  ExcelUtils.WriteExcelSheet("Invoice from Budget Job",EnvParams.Opco,"Data Management",JobNum)
  TextUtils.writeLog("Client Invoice No: "+textobj);
...
function Check_Hitpoint_Status(){ 
  ...
  ExcelUtils.setExcelName(workBook,"Data Management", true);
  ExcelUtils.WriteExcelSheet("Invoice from Budget No",EnvParams.Opco,"Data Management",Invoice_Number)
  ExcelUtils.WriteExcelSheet("Invoice from Budget Job",EnvParams.Opco,"Data Management",jobNumber)
