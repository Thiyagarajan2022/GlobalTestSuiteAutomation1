...
var sheetName ="InterCompany Quote";
...
sheetName ="InterCompany Quote";  
  ...
  if((jobNumber=="")||(jobNumber==null))
  ValidationUtils.verify(false,true,"Job Number is needed to Create Quote for InterCompany Job");
  

  
  ExcelUtils.setExcelName(workBook, "Data Management", true);
  RevisionNo = ReadExcelSheet("InterCompany Budget Revision No",EnvParams.Opco,"Data Management");
  if((RevisionNo=="")||(RevisionNo==null)){
  ExcelUtils.setExcelName(workBook, sheetName, true);
  RevisionNo = ExcelUtils.getColumnDatas("Revision",EnvParams.Opco)
  }
  if((RevisionNo=="")||(RevisionNo==null))
  ValidationUtils.verify(false,true,"Revision Number is needed to Create Quote for InterCompany Job");
}
...
//var Approe =Aliases.Maconomy.Shell.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite9.Composite.PTabFolder.Composite.SingleToolItemControl4;
//else
//var Approe =Aliases.Maconomy.Shell.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite9.Composite.PTabFolder.SWTObject("TabFolderPanel", "", 1).SWTObject("Composite", "", 1).SWTObject("SingleToolItemControl", "", 7);
...
for(var i=0;i<Approve_Level.length;i++){
  ...
  if((Cred[j].indexOf("IND")==-1)&&(Cred[j].indexOf("SPA")==-1)&&(Cred[j].indexOf("SGP")==-1)&&(Cred[j].indexOf("MYS")==-1)&&(Cred[j].indexOf("FP")==-1)&&(Cred[j].indexOf("CHFP")==-1)&&(Cred[j].indexOf("SSC - ")==-1)&&(Cred[j].indexOf("Central Team - Client Management")==-1) &&(Cred[j].indexOf("Central Team - Vendor Management")==-1) && ((Cred[j].indexOf("OpCo - ")!=-1) || (Cred[j].indexOf(Intercompany_OpCo+" ")!=-1)))
  { 
...
if(ImageRepository.ImageSet.Tab_Icon.Exists()){ }
aqUtils.Delay(5000,"Maconomy is loading data tables");
