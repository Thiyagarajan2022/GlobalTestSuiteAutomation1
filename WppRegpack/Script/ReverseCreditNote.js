...
/** 
 * This script reverse created credit note
...
TextUtils.writeLog("Reverse Credit Note PO is Started"); 
...
TextUtils.writeLog("Creating Purchase Order for ReverseCredit Note");
CreatePO = true;
...
CreatePO = false;
ExcelUtils.setExcelName(workBook, "Data Management", true);
Approved_POnumber = ReadExcelSheet("Approved ReverseCredit PO Number",EnvParams.Opco,"Data Management");
if((Approved_POnumber==null)||(Approved_POnumber=="")){ 
TextUtils.writeLog("Approving Purchase Order for ReverseCredit Note");
CreatePO = true;
...
SubmitPurchase.Click();
  ...
  ExcelUtils.setExcelName(workBook,"Data Management", true);
  ExcelUtils.WriteExcelSheet("Reverse Credit PO Number",EnvParams.Opco,"Data Management",PurchaseNumber)
...
ExcelUtils.setExcelName(workBook, "Data Management", true);
POnumber = ReadExcelSheet("Reverse Credit PO Number",EnvParams.Opco,"Data Management");
...
aqUtils.Delay(5000, "Waiting for Action");
InvoiceType.Click();
Log.Message(JavaClasses.MLT.MultiLingualTranslator.GetTransText(Project.Path,Language, "Invoice").OleValue.toString().trim())
WorkspaceUtils.DropDownList(JavaClasses.MLT.MultiLingualTranslator.GetTransText(Project.Path,Language, "Invoice").OleValue.toString().trim(),"Create Method");
aqUtils.Delay(2000, "Waiting for Action");
...
ExcelUtils.setExcelName(workBook,"Data Management", true);
ExcelUtils.WriteExcelSheet("Reverse CreditNote Invoice Journal NO",EnvParams.Opco,"Data Management",journalNumber);
ExcelUtils.WriteExcelSheet("Reverse CreditNote Vendor Invoice NO",EnvParams.Opco,"Data Management",InvoiceNo);
