...
var ReverseJournal = Aliases.Maconomy.BatchAccrual.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite2.Composite.PTabFolder.TabFolderPanel.Composite.markBatchAccruals;
//var Copy = Aliases.Maconomy.Composite.SingleToolItemControl2;
//var ReverseJournal = Aliases.Maconomy.ReverseJournal.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite2.Composite.PTabFolder.TabFolderPanel.Composite.SingleToolItemControl;
Sys.HighlightObject(ReverseJournal);
ReverseJournal.Click();
aqUtils.Delay(5000, "finding Entry Date");
if(Entrydate!=""){
    var date1=Sys.Process("Maconomy").SWTObject("Shell", JavaClasses.MLT.MultiLingualTranslator.GetTransText(Project.Path,Language, "Create Reverse Journal").OleValue.toString().trim()).SWTObject("Composite", "").SWTObject("Composite", "").SWTObject("Composite", "", 1).SWTObject("Composite", "", 1).SWTObject("Composite", "").SWTObject("Composite", "").SWTObject("McPaneGui$10", "").SWTObject("Composite", "").SWTObject("McGroupWidget", "", 1).SWTObject("Composite", "", 1).SWTObject("McDatePickerWidget", "", 2);
...
var crgl=Sys.Process("Maconomy").SWTObject("Shell", JavaClasses.MLT.MultiLingualTranslator.GetTransText(Project.Path,Language, "Create Reverse Journal").OleValue.toString().trim()).SWTObject("Composite", "").SWTObject("Composite", "").SWTObject("Composite", "", 2).SWTObject("Composite", "").SWTObject("Button", JavaClasses.MLT.MultiLingualTranslator.GetTransText(Project.Path,Language, "Create Reverse Journal").OleValue.toString().trim());
...
ReportUtils.logStep("INFO","ReserveJournalNo :"+newjournal);
...
ValidationUtils.verify(true,flag,"Reverse Journal Number is available to submit:"+JournalNumber1)
  if(flag){
...
ValidationUtils.verify(flag,true,"Created Reverse Journal is available in Approval List");
TextUtils.writeLog("Created Reverse Journal is available in Approval List");
if(flag){ 
closefilter.HoverMouse();
ReportUtils.logStep_Screenshot();
closefilter.Click();
...
Post.Click();
  ...
  WorkspaceUtils.savePDF_To_LocalDirectory("PDF ReverseAGeneralJouranl","Print Posting Journal");
