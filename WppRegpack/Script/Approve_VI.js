...
var Language = "";
var VInum = "";
function ApproveInvoice(sheet,VIno){ 
...
InvoiceNo = ReadExcelSheet("Vendor Invoice NO_"+VInum,EnvParams.Opco,"Data Management");
...
//  aqUtils.Delay(3000, Indicator.Text);;
  ...
  Aliases.Maconomy.CreateClient.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite4.Composite.PTabFolder.TabFolderPanel.TabControl
...
if(ImageRepository.ImageSet.Tab_Icon.Exists()){ }
aqUtils.Delay(1000, "Objects are loading");


var action = Aliases.Maconomy.Shell.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite2.Composite2.PTabFolder.Composite2.SWTObject("GroupToolItemControl", "", 7)
action.Click();
aqUtils.Delay(2000, "Waiting for Action");
action.Click();

action.PopupMenu.Click(JavaClasses.MLT.MultiLingualTranslator.GetTransText(Project.Path,Language, "Approve all").OleValue.toString().trim());
if(ImageRepository.ImageSet.Tab_Icon.Exists()){ }
...
//    if((Approve.Child(i).isVisible())&&(Approve.Child(i).text==JavaClasses.MLT.MultiLingualTranslator.GetTransText(Project.Path,Language, "Approve all").OleValue.toString().trim())){
...
//  }
//  if(!st) { 
// Approve = Aliases.Maconomy.Shell.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite2.Composite2.PTabFolder.Composite2;
// Sys.HighlightObject(Approve)
// for(var i=0;i<Approve.ChildCount;i++){ 
//  if((Approve.Child(i).isVisible())&&(Approve.Child(i).text==JavaClasses.MLT.MultiLingualTranslator.GetTransText(Project.Path,Language, "Approve all").OleValue.toString().trim())){
//    Approve = Approve.Child(i);
...
var loginPer = Sys.Process("Maconomy").SWTObject("Shell", "Deltek Maconomy - *").WndCaption;
    ...
    var i=0;
while ((((ApvPerson.getText().OleValue.toString().trim().indexOf(JavaClasses.MLT.MultiLingualTranslator.GetTransText(Project.Path,Language, "pproved").OleValue.toString().trim())==-1)&&(ApvPerson.getText().OleValue.toString().trim().toUpperCase().indexOf(JavaClasses.MLT.MultiLingualTranslator.GetTransText(Project.Path,Language, "YOU").OleValue.toString().trim())==-1))&&(ApvPerson.getText().OleValue.toString().trim().indexOf(loginPer)==-1))&&(i!=600))
{
  aqUtils.Delay(100);
  i++;
  ApvPerson.Refresh();
...
ExcelUtils.setExcelName(workBook,"Data Management", true);
ExcelUtils.WriteExcelSheet("Approved Vendor Invoice_"+VInum,EnvParams.Opco,"Data Management",InvoiceNo);
...
var invoiceapproval = 
//Aliases.Maconomy.Shell.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.PTabFolder.TabFolderPanel.TabControl;
Aliases.Maconomy.Shell.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.PTabFolder.TabFolderPanel.SWTObject("TabControl", "", 12)
WorkspaceUtils.waitForObj(invoiceapproval);
...
Aliases.Maconomy.Shell.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.PTabFolder.SWTObject("Composite", "", 11).SWTObject("McClumpSashForm", "").SWTObject("Composite", "", 1).SWTObject("Composite", "").SWTObject("McTableWidget", "").SWTObject("McGrid", "", 2)
