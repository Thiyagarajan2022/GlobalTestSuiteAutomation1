...
ValidationUtils.verify(false,true,"Client Number is Needed to Block Company Product");
}
...
ValidationUtils.verify(false,true,"Product Number is Needed to Block Company Product");
}

  ExcelUtils.setExcelName(workBook, "Data Management", true);
  ProductName = ReadExcelSheet("Global Product Name",EnvParams.Opco,"Data Management");
...
ValidationUtils.verify(false,true,"Product Name is Needed to Block Company Product");
}
ExcelUtils.setExcelName(workBook, sheetName, true);
Currency = ExcelUtils.getRowDatas("Currency",EnvParams.Opco)
if((Currency==null)||(Currency=="")){ 
ValidationUtils.verify(false,true,"Currency is Needed to Block Company Product");
}
...
if(ImageRepository.ImageSet.Tab_Icon.Exists()){ 
  ...
  var activeProduct = Aliases.Maconomy.Screen.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.PTabFolder.Composite.McClumpSashForm.Composite.Composite.McFilterPaneWidget.McFilterContainer.Composite.McFilterPanelWidget.SWTObject("Button", JavaClasses.MLT.MultiLingualTranslator.GetTransText(Project.Path,Language, "Active Company Products").OleValue.toString().trim())
  activeProduct.Click();
  aqUtils.Delay(3000, Indicator.Text);
