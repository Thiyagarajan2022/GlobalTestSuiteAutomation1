...
function getDetails(){
  ...
  fileName = ExcelUtils.getRowDatas("Bank_Reconciliation PDF",EnvParams.Opco)
  ...
  statementDate = ExcelUtils.getRowDatas("Statement Date",EnvParams.Opco)
  Log.Message(statementDate);
  if((statementDate==null)||(statementDate=="")){ 
  ValidationUtils.verify(false,true,"StatementDate is needed to validate");
  }
  
  ExcelUtils.setExcelName(workBook, "Banking Reconciliation", true);
  statementNo = ExcelUtils.getRowDatas("Statement No",EnvParams.Opco)
  Log.Message(statementNo);
  if((statementNo==null)||(statementNo=="")){ 
  ValidationUtils.verify(false,true,"statementNo is needed to validate");
  }
...
function Banking_Reconciliation()
{
...
//          
     ...
     if(index>=0){
          ...
          ValidationUtils.verify(true,true,"Statement Number is available in Pdf")
          TextUtils.writeLog("Statement Number is available in Pdf")
          }
          else
          ValidationUtils.verify(false,true,"Statement Number is not available in Pdf")             
