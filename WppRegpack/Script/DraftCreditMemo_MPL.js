...
function DraftCreditMemo()
{
  ...
  ValidationUtils.verify(false,true,"Draft Credit Memo PDF is needed to validate");
  }
  var docObj;
...
productName = ReadExcelSheet("Product Name",EnvParams.Opco,"CreateClient");
   ...
   Log.Message(pdflineSplit)
    ...
    if(index>=0){
          ...
          ValidationUtils.verify(false,true,"ClientName is not same in Draft Credit Note");
      ...
      if(pdflineSplit[j].includes("Product Name"))
        {
           ...
           ReportUtils.logStep("INFO",productName+" ProductName is matching with Pdf")
              ...
              ValidationUtils.verify(false,true,"ProductName is not same in Draft Credit Note");            
        }
        }
      }
   ...
   var index = pdflineSplit.indexOf(street);
    if(index>=0){
          ...
          ValidationUtils.verify(false,true,"Street is not same in Draft Credit Note");
   var index = pdflineSplit.indexOf(postCode+"  "+postDistrict);
    ...
    if(index>=0){
          ...
          ValidationUtils.verify(false,true,"PostCode and Post District are not same in Draft Credit Note");
   var index = pdflineSplit.indexOf(country);
    if(index>=0){
          ...
          ValidationUtils.verify(false,true,"Country is not same in Draft Credit Note");
  ...
  if((jobNumber=="")||(jobNumber==null))
  ValidationUtils.verify(false,true,"Job Number is needed to Validate Draft Credit Note");
 ...
 Log.Message(pdflineSplit.length)
  for (j=0; j<pdflineSplit.length; j++)
  {
      ...
      if(pdflineSplit[j].includes(JavaClasses.MLT.MultiLingualTranslator.GetTransText(Project.Path,Language, "Attn.").OleValue.toString().trim()))
    {
      ...
      pdfJobNum = x[1].trim();
       if(Attn!=pdfJobNum)
        ValidationUtils.verify(false,true,"Attention is not same in Draft Credit Note");
        else{
    ...
    if((EnvParams.Country.toUpperCase()=="CHINA")&&(Language=="Chinese (Simplified)")){
      ...
      pdfJobNum = x[1].trim();
       if("xxxxxx"!=pdfJobNum)
        ValidationUtils.verify(false,true,"Draft Credit Note Number is not same in Draft Credit Note");
        else{
        ReportUtils.logStep("INFO","xxxxxx Draft Credit Note Number is matching with Pdf")
        ValidationUtils.verify(true,true,"xxxxxx Draft Credit Note Number is matching with Pdf")
        TextUtils.writeLog("xxxxxx Draft Credit Note Number is matching with Pdf")
        }
    }
   ...
   if(pdflineSplit[j].includes(JavaClasses.MLT.MultiLingualTranslator.GetTransText(Project.Path,Language, "Credit Note Date").OleValue.toString().trim()))
    {   
    Log.Message("CreditNote Date :"+pdflineSplit[j])
    if((EnvParams.Country.toUpperCase()=="CHINA")&&(Language=="Chinese (Simplified)")){
      ...
      pdfJobNum = x[1].trim();
       if("xxxxxx"!=pdfJobNum)
        ValidationUtils.verify(false,true,"Credit Note Date is not same in Draft Credit Note");
        else{
        ReportUtils.logStep("INFO","xxxxxx Credit Note Date is matching with Pdf")
        ValidationUtils.verify(true,true,"xxxxxx Credit Note Date is matching with Pdf")
        TextUtils.writeLog("xxxxxx Credit Note Date is matching with Pdf")
        }
    }
    ...
    if((EnvParams.Country.toUpperCase()=="CHINA")&&(Language=="Chinese (Simplified)")){
      ...
      pdfJobNum = x[1].trim();
       if("xxxxxx"!=pdfJobNum)
        ValidationUtils.verify(false,true,"Due Date is not same in Draft Credit Note");
        else{
    ...
    if((EnvParams.Country.toUpperCase()=="CHINA")&&(Language=="Chinese (Simplified)")){
      ...
      Log.Message(pdfJobNum)
       if(pdfJobNum.indexOf(PaymentTerm)==-1)
        ValidationUtils.verify(false,true,"Payment Terms is not same in Draft Credit Note");
        else{
        ...
        if(pdflineSplit[j].includes(clientNumber))
         {
          ...
          ValidationUtils.verify(false,true,"Client Number is not same in Draft Credit Note");
          if(EnvParams.Country.toUpperCase()=="INDIA")
           break;
        }
    }      
   ...
   if(EnvParams.Country.toUpperCase()=="INDIA"){
             ...
             if(pdflineSplit[j].includes("GSTIN"))
              {
               ...
               if(clientGST!=pdfClientGST)
                ValidationUtils.verify(false,true,"clientGST is not same in Draft Credit Note");
               else
               {
             ...
             if(pdflineSplit[j].includes("Place of Supply"))
              {
               ...
               {
                ValidationUtils.verify(false,true,"POS is not same in Draft Credit Note");
                break;
               }
            }
       } 
    ...
    var cin = ReadExcelSheet("CIN/UIN",EnvParams.Opco,"OpCo Details");
      ...
      if(pdflineSplit[j].includes("PAN"))
      {
        ...
        pdfPan = x[1].trim();
         if(pan!=pdfPan)
          ValidationUtils.verify(false,true,"PAN is not same in Draft Credit Note");
         else{
  ...
  temp = temp.trim();
   for (z=0; z<pdflineSplit.length; z++){
    ...
    if(index>=0){
          ...
          ValidationUtils.verify(false,true,"Total is not same in Draft Credit Note");
}
