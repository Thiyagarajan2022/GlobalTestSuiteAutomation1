...
function getDetails(){
  ...
  ValidationUtils.verify(false,true,"PrintPaymentRemittanceMpl is needed to validate");
  }
...
if((Paymentagent==null)||(Paymentagent=="")){ 
ValidationUtils.verify(false,true,"Payment Agent is Needed to validate PrintReimmittance PDF");
}
...
if((Paymodemode==null)||(Paymodemode=="")){ 
ValidationUtils.verify(false,true,"Paymode Mode Number is Needed to validate PrintReimmittance PDF");
}
...
Duedate = ExcelUtils.getRowDatas("Vendor Invoice Due Date",EnvParams.Opco)
  if((Duedate==null)||(Duedate==""))  { 
    ExcelUtils.setExcelName(workBook, sheetName, true);
    Duedate = ExcelUtils.getRowDatas("Payment Date",EnvParams.Opco)
  }
Log.Message(Duedate)
if((Duedate==null)||(Duedate=="")){ 
ValidationUtils.verify(false,true,"Due Date Number is Needed to validate PrintReimmittance PDF");
}
...
Log.Message(amount)
if((amount==null)||(amount=="")){ 
ValidationUtils.verify(false,true,"Amount is Needed to validate PrintReimmittance PDF");
}
...
ValidationUtils.verify(false,true,"Layout is Needed to validate PrintReimmittance PDF");
}

ExcelUtils.setExcelName(workBook, sheetName, true);
exchangeDate = ExcelUtils.getRowDatas("ExchangeDate",EnvParams.Opco)
Log.Message(exchangeDate)
if((exchangeDate==null)||(exchangeDate=="")){ 
ValidationUtils.verify(false,true,"exchangeDate is Needed to validate PrintReimmittance PDF");
}
...
if((Invoicenumber==null)||(Invoicenumber=="")){ 
ValidationUtils.verify(false,true,"Vendor Invoice Number is Needed to validate PrintReimmittance PDF");
}
...
ValidationUtils.verify(false,true,"Vendor Number is Needed to validate PrintReimmittance PDF");
}

ExcelUtils.setExcelName(workBook, "Data Management", true);
paymentNumber = ExcelUtils.getRowDatas("Payment Number",EnvParams.Opco)
if((paymentNumber=="")||(paymentNumber==null)){
ExcelUtils.setExcelName(workBook, sheetName, true);
paymentNumber = ExcelUtils.getRowDatas("Payment_Number",EnvParams.Opco)
Log.Message(paymentNumber)
}
if((paymentNumber==null)||(paymentNumber=="")){ 
ValidationUtils.verify(false,true,"Payment Number is Needed to validate PrintReimmittance PDF");
}
...
function validatePaymentRemittance_standardLayout()
{ 
  var docObj;
  try{
  ...
  verifyPaymentNumber(paymentNumber, pdflineSplit);
...
function validatePaymentRemittance_wppLayout()
{
  var docObj;
...
//Main Function
function validatePaymentRemittanceMPL() {
TextUtils.writeLog("Payment Remitance MPL Validation Started"); 
Language = "";
...
{
  validatePaymentRemittance_wppLayout();
}
else if(layoutTypes=="Standard")
{
  validatePaymentRemittance_standardLayout();
}
...
function verifyVendorNumber(vendorNumber,pdflineSplit)
{
  ...
  for (var j=0; j<pdflineSplit.length; j++)
  {
         ...
         if(j==pdflineSplit.length-1 && !vendorNoFound)
          ValidationUtils.verify(false,true,"VendorNumber is not same in CPrintReimmittance");
  }  
}
...
function verifyInvoiceNumber(vendorInvoiceNo,pdflineSplit)
{
  ...
  for (var j=0; j<pdflineSplit.length; j++)
  {
         ...
         if(j==pdflineSplit.length-1 && !vendorInvoiceNoFound)
          ValidationUtils.verify(false,true,"vendorInvoiceNo is not same in PrintReimmittance");
  ...
  var amountFound = false;
    for (var j=0; j<pdflineSplit.length; j++)
    {
         ...
         if(j==pdflineSplit.length-1 && !amountFound)
          ValidationUtils.verify(false,true,"amount is not same in PrintReimmittance");
  ...
  var exchangeDateFound = false;
    for (var j=0; j<pdflineSplit.length; j++)
    {
         ...
         if(j==pdflineSplit.length-1 && !exchangeDateFound)
          ValidationUtils.verify(false,true,"exchangeDate is not same in PrintReimmittance");
...
function verifyDueDate(dueDate,pdflineSplit)
{
    ...
    for (var j=0; j<pdflineSplit.length; j++)
    {
         ...
         if(j==pdflineSplit.length-1 && !dueDateFound)
          ValidationUtils.verify(false,true,"DueDate is not same in PrintReimmittance");
   ...
   var paymodeModeFound = false;
    for (var j=0; j<pdflineSplit.length; j++)
    {
         ...
         if(j==pdflineSplit.length-1 && !paymodeModeFound)
          ValidationUtils.verify(false,true,"paymodeMode is not same in PrintReimmittance");    
    }
}
   ...
   var paymentAgentFound = false;
    for (var j=0; j<pdflineSplit.length; j++)
    {
         ...
         if(j==pdflineSplit.length-1 && !paymentAgentFound)
          ValidationUtils.verify(false,true,"paymentAgent is not same in PrintReimmittance");    
    }
}
   ...
   var paymentNumberFound = false;
    for (var j=0; j<pdflineSplit.length; j++)
    {
         if(pdflineSplit[j].includes(paymentNumber))
             ...
             ValidationUtils.verify(true,true,"PaymentNumber is matched in with Pdf")
