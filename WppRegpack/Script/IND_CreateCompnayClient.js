...
var SaveStat = false;
  ...
  var spainspec = Aliases.Maconomy.Shell.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.PTabFolder.TabFolderPanel;
  if(ImageRepository.ImageSet.Tab_Icon.Exists()){ 
    
  }
  Sys.HighlightObject(spainspec)
  
  for(var i =0 ;i<spainspec.ChildCount;i++){ 
  if((spainspec.Child(i).Visible)&&(spainspec.Child(i).ChildCount==0)) { 
    if(spainspec.Child(i).text=="India Specific"){ 
      indiaspec = spainspec.Child(i);
      break;
    }
