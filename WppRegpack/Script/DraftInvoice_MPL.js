...
function DraftInvoice()
{
  ...
  fileName = ExcelUtils.getRowDatas("PDF Draft Invoice",EnvParams.Opco)
...
var pName = false;  
 Log.Message(pdflineSplit.length)
  for (j=1; j<35; j++)
  {
    ...
    if((EnvParams.Country.toUpperCase()=="CHINA")&&(Language=="Chinese (Simplified)")){
      ...
      pdfJobNum = x[1].trim();
       if("xxxxxx"!=pdfJobNum)
        ...
        ReportUtils.logStep("INFO","xxxxxx Invoice Number is matching with Pdf")
        ValidationUtils.verify(true,true,"xxxxxx Invoice Number is matching with Pdf")
        TextUtils.writeLog("xxxxxx Invoice Number is matching with Pdf")
        }
    }
    ...
    if((EnvParams.Country.toUpperCase()=="CHINA")&&(Language=="Chinese (Simplified)")){
        var atSize = JavaClasses.MLT.MultiLingualTranslator.GetTransText(Project.Path, Language, "Invoice Date").OleValue.toString().trim();
      pdflineSplit[j] = pdflineSplit[j].substring(atSize.length+1); 
      x= pdflineSplit[j].split(" ");
      x[0]= pdflineSplit[j];
      x[1]= pdflineSplit[j];
    ...
    if((EnvParams.Country.toUpperCase()=="CHINA")&&(Language=="Chinese (Simplified)")){
      ...
      pdfJobNum = x[1].trim();
       if("xxxxxx"!=pdfJobNum)
        ValidationUtils.verify(false,true,"Due Date is not same in Draft Invoice");
        else{
