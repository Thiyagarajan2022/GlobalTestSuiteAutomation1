...
function verifyARReceiptRecon_Report(){
TextUtils.writeLog("Verification Of AR Receipt Reconciliation Report"); 
...
gotoMenu(); 
gotoARScreen();
gotoARReceiptReconciliationLink();
verifyARReceiptReconciliationScreen();
}
  catch(err){
    Log.Message(err);
  }
...
function gotoARReceiptReconciliationLink()
{
var arReceiptLink = Aliases.Maconomy.Reports_GL.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.PTabFolder.Composite3.McClumpSashForm.Composite.Composite.McPaneGui_10.Composite.Composite.Composite.McGroupWidget.Composite3.McLinkLabelWidget.McTextWidget;
waitForObj(arReceiptLink);
Sys.HighlightObject(arReceiptLink);
ReportUtils.logStep_Screenshot();
arReceiptLink.Click();
ReportUtils.logStep("INFO", "Clicked on AR Receipt Reconciliation Link");
Log.Message("Clicked on AR Receipt Reconciliation Link");
if(ImageRepository.ImageSet.LoadedBox.Exists())
{}
}


function verifyARReceiptReconciliationScreen()
{
