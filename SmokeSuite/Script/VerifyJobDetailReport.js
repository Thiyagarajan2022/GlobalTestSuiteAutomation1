...
function verifyJobDetail_Report(){
TextUtils.writeLog("Verification Of Job Detail Report"); 
...
function gotoJobDetailLink()
{
var jobDetailLink = Aliases.Maconomy.Reports_GL.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.PTabFolder.Composite4.McClumpSashForm.Composite.Composite.McPaneGui_10.Composite.Composite.Composite.McGroupWidget.Composite4.McLinkLabelWidget.McTextWidget;
waitForObj(jobDetailLink);
Sys.HighlightObject(jobDetailLink);
ReportUtils.logStep_Screenshot();
jobDetailLink.Click();
ReportUtils.logStep("INFO", "Clicked on Job Detail Link");
Log.Message("Clicked on Job Detail Link");
}


function verifyJobDetailScreen()
{
  ...
  if(pageName.trim()== "Job Detail" || ImageRepository.Browser_Reporting.JobDetail_Logo.Exists())
  {
     ...
     Log.Message("Job Detail Screen displayed sucessfully");
     } 
  else
