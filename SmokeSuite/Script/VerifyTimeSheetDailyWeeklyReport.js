...
function verifyTimeSheetDailyWeekly_Report(){
TextUtils.writeLog("Verification Of Time Sheet Daily and Weekly Report"); 
...
gotoMenu(); 
gotoTimeScreen();
gotoTimeSheetDailyAndWeeklyLink();
verifyTimeSheetDailyAndWeeklyScreen();
}
  catch(err){
    Log.Message(err);
  }
...
function gotoTimeScreen(){ 

var timeScreen = Aliases.Maconomy.Reports_GL.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.PTabFolder.TabFolderPanel.TabControl4;
waitForObj(timeScreen);
Sys.HighlightObject(timeScreen);
timeScreen.Click();
ReportUtils.logStep("INFO", "Clicked Time Menu Section");
Log.Message("Clicked Time Menu Section")
if(ImageRepository.ImageSet.LoadedBox.Exists())
{}
}

function gotoTimeSheetDailyAndWeeklyLink()
{
var timeSheetLink = Aliases.Maconomy.Reports_GL.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.PTabFolder.Composite5.McClumpSashForm.Composite.Composite.McPaneGui_10.Composite.Composite.Composite.McGroupWidget.Composite2.McLinkLabelWidget.McTextWidget;
waitForObj(timeSheetLink);
Sys.HighlightObject(timeSheetLink);
ReportUtils.logStep_Screenshot();
timeSheetLink.Click();
ReportUtils.logStep("INFO", "Clicked on TimeSheet Daily And Weekly Link");
Log.Message("Clicked on TimeSheet Daily And Weekly Link");
}


function verifyTimeSheetDailyAndWeeklyScreen()
{
 ...
 else if(pageName.trim() == "Timesheet Daily & Weekly" || ImageRepository.Browser_Reporting.TimeSheetDailyWeekly_1_Logo.Exists())
  {
     ReportUtils.logStep_Screenshot();
     ReportUtils.logStep("Pass", "TimeSheet Daily And Weekly Screen displayed sucessfully");
     Log.Message("TimeSheet Daily And Weekly Screen displayed sucessfully");
