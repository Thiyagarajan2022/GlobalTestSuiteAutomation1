...
function verifyTaxSettlementStatutory_Report(){
TextUtils.writeLog("Verification Of Tax Settlement Statutory Report"); 
...
gotoMenu(); 
gotoTaxSettlementStatutoryLink();
verifyTaxSettlementStatutoryScreen();
}
  catch(err){
    Log.Message(err);
  }
...
function gotoTaxSettlementStatutoryLink()
{
var taxSettlementLink = Aliases.Maconomy.Reports_GL.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.Composite.PTabFolder.Composite.McClumpSashForm.Composite.Composite.McPaneGui_10.Composite.Composite.McGroupWidget.Composite2.McLinkLabelWidget.McTextWidget;
waitForObj(taxSettlementLink);
Sys.HighlightObject(taxSettlementLink);
ReportUtils.logStep_Screenshot();
taxSettlementLink.HoverMouse();
taxSettlementLink.Click();
ReportUtils.logStep("INFO", "Clicked on Tax Settlement Statutory Link");
Log.Message("Clicked on Tax Settlement Statutory Link");
}


function verifyTaxSettlementStatutoryScreen()
{
