...
/** 
 * This script implements printing of Draft Invoice PDF and verify contents against Maconomy
...
function verifyDraftInvoiceMPL(){
TextUtils.writeLog("Verification Of Draft Invoice MPL"); 
...
jobNumber = ExcelUtils.getRowDatas("Draft_Invoice_JobNumber",EnvParams.Opco)
...
gotoMenu(); 
verify_DraftInvoiceMPL(jobNumber);
}
  catch(err){
    Log.Message(err);
  }
...
/**
  *  This function prints Draft Invoice PDF and validates fields
  */

function verify_DraftInvoiceMPL(jobNumber){ 
...
WorkspaceUtils.waitForObj(allJobsRadio);
...
printDraftBtn.HoverMouse();
ReportUtils.logStep_Screenshot("");
printDraftBtn.Click();
TextUtils.writeLog("Print Draft Invoice is Clicked and saved"); 
if(ImageRepository.ImageSet.LoadedBox.Exists()){}
