...
function verifyPaymentVoucher_Report(){
TextUtils.writeLog("Verification Of Payment Voucher Report"); 
...
gotoMenu(); 
gotoPaymentVoucherLink();
verifyPaymentVoucherScreen();
}
  catch(err){
    Log.Message(err);
  }
...
ReportUtils.logStep_Screenshot();
paymentVoucherLink.Click();
ReportUtils.logStep("INFO", "Clicked on Payment Voucher Link");
Log.Message("Clicked on Payment Voucher Link");
}


function verifyPaymentVoucherScreen()
{
 ...
 if (ImageRepository.Browser_Reporting.Browser_Reports_Prompt.Exists())  
  ...
  if(ImageRepository.Browser_Reporting.PaymentVoucher_Logo.Exists())
  {
     ...
     Log.Message("Payment Voucher Screen displayed successfully");
     } 
  else
